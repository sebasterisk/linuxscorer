...
class Module(App):
    ...
    def action_update_score(self) -> None:
        with open("score.json", "r") as f:
            ...
            self.query_one("#nvulns").update(f"{data["vulnsCompleted"]} out of {data["vulnsTotal"]} issues addressed")
            self.query_one("#npoints").update(f"{data["pointsAttained"]} out of {data["pointsTotal"]} points scored")
            self.query_one("#timeupdate").update(f"last update: {datetime.datetime.now().ctime()}")
            self.query_one("#prog").update(total = data["pointsAttained"], progress = data["pointsTotal"])

            self.query_one("#list").clear()
            for i in data["completedVulnDescription"]:
